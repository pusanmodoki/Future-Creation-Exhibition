		...
		namespace NodeEditor
		{
			...
			namespace ReorderableLists
			{
				public class ProbabilityList
				{
					...
					public ProbabilityList(BTBaseNodeEditor editor)
					{
						m_this = editor;
						...
						list.elementHeight = EditorGUIUtility.singleLineHeight * 3 + 6;
						list.onReorderCallback += ReorderCallback;
						list.drawHeaderCallback += (rect) => EditorGUI.LabelField(rect, "Children node probabilitys");
						list.drawElementCallback = (rect, index, isActive, isFocused) => {
							...
							rect.position = rect.position + new Vector2(0.0f, EditorGUIUtility.singleLineHeight + 2.0f);
							EditorGUI.Slider(rect, m_this.propertys.probabilitys.GetArrayElementAtIndex(index),
								0.0f, 1.0f, "probability");
						};
					}
