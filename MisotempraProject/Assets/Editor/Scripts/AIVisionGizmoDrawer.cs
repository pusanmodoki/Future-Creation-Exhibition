...
{
	public static class AIVisionGizmoDrawer
	{  /// <summary>
	   /// Mesh保存用構造体
	   /// </summary>
		...
		static void DrawGizmos(AI.AIVision vision, GizmoType gizmoType)
		{
			//null?
			if (vision.aiAgent == null) return;
			//draw?
			if (!vision.dIsDrawGizmos) return;
			...
			if (!EditorApplication.isPlaying || EditorApplication.isPaused)
				vision.DExecuteVision();

			//呼び出しコスト削減
			Transform transform = vision.transform;
			Vector3 position = transform.position;
			Vector3 rayPosition = transform.position + vision.raycastCenter;
			Quaternion rotation = transform.rotation;
			//地面に接触する可能性を考慮し描画位置を調整するために使用
			Vector3 adjustUp = Vector3.up * 0.1f;
			//レイ開始地点
			Vector3 rayStart = rayPosition + adjustUp;

			//視界ヒット
			if (vision.dIsDiscover)
			{
				if (vision.targetTransform != null)
				{
					//レイ終了地点
					Vector3 rayEnd = (vision.targetTransform.position - rayPosition).normalized * vision.raycastHit.distance;
					//レイ用に色変更しレイ描画
			...
			else
			{
				...
				{
					//レイ終了地点
					Vector3 rayEnd = (vision.targetTransform.position - rayPosition).normalized * vision.distance;
					//レイ用に色変更しレイ描画
			...
			if (vision.dGizmoMesh == null)
			{
				...
				if (vision.dMeshID != -1)
					...
					{
						//ID合致でカウンター減算, break
						if (vision.dMeshID == m_meshs[i].id)
						{
							var edit = m_meshs[i];
							edit.counter--;
							m_meshs[i] = edit;
				...
				{
					//同じ角度のMeshがある
					if (vision.angle == m_meshs[i].angle)
					{
						//visibilityの値変更
						vision.dGizmoMesh = m_meshs[i].mesh;
						vision.dMeshID = m_meshs[i].id;
						//カウンター加算, break
						var edit = m_meshs[i];
						edit.counter++;
						m_meshs[i] = edit;
				...
				{
					//Mesh作成
					vision.dGizmoMesh = CreateMesh(vision.angle);
					//メッシュリストに追加
					m_meshs.Add(new SectorMesh(vision.dGizmoMesh, vision.angle, m_meshID));
					//ID代入
					vision.dMeshID = m_meshID;
					//IDを次に進める
					m_meshID++;
				}
			}
			...
			//Mesh描画
			Gizmos.DrawMesh(vision.dGizmoMesh, position + adjustUp,
				rotation * Quaternion.AngleAxis(90.0f, Vector3.forward), Vector3.one * vision.distance);
			Gizmos.DrawMesh(vision.dGizmoMesh, position + adjustUp,
				rotation * Quaternion.AngleAxis(270.0f, Vector3.forward), Vector3.one * vision.distance);
			Gizmos.DrawMesh(vision.dGizmoMesh, position + adjustUp,
				rotation, Vector3.one * vision.distance);

			Gizmos.DrawWireSphere(position, vision.loseSightDistance);
			Gizmos.matrix = Matrix4x4.Translate(position + adjustUp + vision.transform.forward * vision.distance * 0.5f) * Matrix4x4.Rotate(rotation);
			Gizmos.DrawWireCube(Vector3.zero, new Vector3(0.3f, vision.heightLimit, vision.distance));
		}
