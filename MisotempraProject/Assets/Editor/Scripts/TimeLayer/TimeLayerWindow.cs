...
namespace Editor
{
	namespace TimeLayer
	{
		...
		{
			[System.Serializable]
			class DataInstance : ScriptableObject
			{
				/// <summary>プロパティで使用するセーブレイヤーリスト</summary>
				public ReadOnlyCollection<SaveTimeLayer> readSaveLayers { get; private set; } = null;
				/// <summary>プロパティで使用するレイヤーリスト(key save layer)</summary>
				public ReadOnlyDictionary<SaveTimeLayer, TimeManagement.TimeLayer> readLayers { get; private set; } = null;
				/// <summary>プロパティで使用するレイヤーリスト(key guid)</summary>
				public ReadOnlyDictionary<string, TimeManagement.TimeLayer> readLayersKeyGuid { get; private set; } = null;
				/// <summary>プロパティで使用するレイヤーインデックスリスト(key guid)</summary>
				public ReadOnlyDictionary<string, int> readLayerIndexesKeyGuid { get; private set; } = null;
				/// <summary>プロパティで使用するネームリスト</summary>
				public string[] usePropertyLayerNames { get; private set; } = null;

				/// <summary>セーブ用レイヤー</summary>
				public List<SaveTimeLayer> saveLayers = new List<SaveTimeLayer>();
				/// <summary>レイヤー</summary>
				public Dictionary<SaveTimeLayer, TimeManagement.TimeLayer> layers = new Dictionary<SaveTimeLayer, TimeManagement.TimeLayer>();
				...
				public Dictionary<string, TimeManagement.TimeLayer> layersKeyGuid = new Dictionary<string, TimeManagement.TimeLayer>();
				/// <summary>プロパティ用リスト</summary>
				public Dictionary<string, int> layerIndexesKeyGuid = new Dictionary<string, int>();
				...
				public void DataLoad()
				{
					...
					{
						try { FileAccessor.LoadObject(path, name, out saveLayers, TimeManager.cFileBeginMark); }
						catch (System.Exception e) { Debug.LogError(e.Message); throw; }
					}
					else
					{
						saveLayers = new List<SaveTimeLayer>();
						saveLayers.Add(new SaveTimeLayer("root", "", new List<string>(), 1.0f));
						FileAccessor.SaveObject(path, name, ref saveLayers, TimeManager.cFileBeginMark);
					}

					//レイヤー追加ループ
					foreach (var e in saveLayers)
						layers.Add(e, new TimeManagement.TimeLayer(e.name, e.timeScale, e.guid));
					//親子設定ループ
					foreach (var e in saveLayers)
					{
						//親がいれば
						...
						{
							//親探査ループ
							foreach (var find in layers)
							{
								//親が見つかれば設定
								if (e.parentGuid == find.Value.guid)
								{
									layers[e].callManagerFunctions.SetParent(find.Value);
									break;
								}
							}
							//失敗？
							if (e.parentGuid.Length > 0 && layers[e].parent == null)
						...
						for (int i = 0; i < e.childrensGuid.Count; ++i)
						{
							...
							foreach (var find in layers)
							{
								...
								{
									layers[e].callManagerFunctions.AddChildren(find.Value);
									break;
								}
							}
						}
					...
					//New
					readSaveLayers = new ReadOnlyCollection<SaveTimeLayer>(saveLayers);
					readLayers = new ReadOnlyDictionary<SaveTimeLayer, TimeManagement.TimeLayer>(layers);
					readLayersKeyGuid = new ReadOnlyDictionary<string, TimeManagement.TimeLayer>(layersKeyGuid);
					readLayerIndexesKeyGuid = new ReadOnlyDictionary<string, int>(layerIndexesKeyGuid);
				}
				...
				public void UpdatePropertyInfo()
				{
					...
					{
						string layerName = layers[saveLayers[i]].name;
						int sameCounter = 0;
						for (int k = 0; saveLayers[k].guid != layers[saveLayers[i]].guid; ++k)
						{
							if (saveLayers[k].name == layers[saveLayers[i]].name)
								++sameCounter;
						}
						if (sameCounter > 0) layerName += " (" + sameCounter + ")";

						result.Add(layerName);
						guidToIndexes.Add(layers[saveLayers[i]].guid, i);
					}
					...
					//リスト構築
					layersKeyGuid.Clear();
					layerIndexesKeyGuid.Clear();
					for (int i = 0; i < saveLayers.Count; ++i)
					{
						layersKeyGuid.Add(saveLayers[i].guid, layers[saveLayers[i]]);
						layerIndexesKeyGuid.Add(saveLayers[i].guid, i);
					}
				}
			}
			...
			static DataInstance m_dataInstance = null;
			...
			/// <summary>プロパティで使用するセーブレイヤーリスト</summary>
			public static ReadOnlyCollection<SaveTimeLayer> saveLayers { get { return m_dataInstance.readSaveLayers; } }
			/// <summary>プロパティで使用するレイヤーリスト(key save layer)</summary>
			public static ReadOnlyDictionary<SaveTimeLayer, TimeManagement.TimeLayer> layers { get { return m_dataInstance.readLayers; } }
			/// <summary>プロパティで使用するレイヤーリスト(key guid)</summary>
			public static ReadOnlyDictionary<string, TimeManagement.TimeLayer> layersKeyGuid { get { return m_dataInstance.readLayersKeyGuid; } }
			/// <summary>プロパティで使用するレイヤーインデックスリスト(key guid)</summary>
			public static ReadOnlyDictionary<string, int> layerIndexesKeyGuid { get { return m_dataInstance.readLayerIndexesKeyGuid; } }
			/// <summary>プロパティで使用するネームリスト</summary>
			public static string[] usePropertyLayerNames { get { return m_dataInstance.usePropertyLayerNames; } }

			/// <summary>セーブ用レイヤー</summary>
			List<SaveTimeLayer> m_saveLayers { get { return m_dataInstance.saveLayers; } }
			/// <summary>レイヤー</summary>
			Dictionary<SaveTimeLayer, TimeManagement.TimeLayer> m_layers { get { return m_dataInstance.layers; } }
			/// <summary>各レイヤーレクト, key = guid</summary>
			Dictionary<string, Rect> m_windowRects { get { return m_dataInstance.windowRects; } }
			/// <summary>プロパティ用リスト</summary>
			Dictionary<string, TimeManagement.TimeLayer> m_layersKeyGuid { get { return m_dataInstance.layersKeyGuid; } }
			/// <summary>プロパティ用リスト</summary>
			Dictionary<string, int> m_layerIndexesKeyGuid { get { return m_dataInstance.layerIndexesKeyGuid; } }
			...
			bool m_isAddSaveCallback = false;
			...
			public static void IfEmptyLoading()
			{
				if (m_dataInstance == null) m_dataInstance = CreateInstance<DataInstance>();
				if (m_dataInstance.saveLayers.Count == 0) m_dataInstance.DataLoad();
			}
			...
			void OnGUI()
			{
				...
				if (EditorGUI.EndChangeCheck()) Save(false);
			}
			...
			void Save(bool isDrawLog)
			{
				...
				{
					//セーブ
					FileAccessor.SaveObject(TimeManager.savePath, TimeManager.cSaveFileName,
						ref m_dataInstance.saveLayers, TimeManager.cFileBeginMark);
					if (isDrawLog) Debug.Log("TimeLayer Editor save completed.");
				}
				catch (System.Exception e)
				{
					Debug.LogError("TimeLayer Editor save failed.\n message: " + e.Message);
				}
			}
			...
			void LoadingAddWindowRect(TimeManagement.TimeLayer layer, float x, float y, List<int> windowPositionIndex, int depth)
			{
				//ウィンドウ追加
					...
					case 1:
						{
							...
							if (onWindow < 0)
							{
								...
								//メニュー追加
								menu.AddItem(new GUIContent("Save(Auto-save is enabled.)"), false, MenuCallback, m_cMenuModeSave);
			...
			void AddChildrenGuid(TimeManagement.TimeLayer layer, List<string> list)
			{
				//追加
				list.Add(layer.guid);
				//子供でも実行
					...
					case m_cMenuModeCreateNode:
						{
							...
							m_layers.Add(m_saveLayers[m_saveLayers.Count - 1],
								new TimeManagement.TimeLayer(m_saveLayers[m_saveLayers.Count - 1].name,
							...
							//更新
							m_dataInstance.UpdatePropertyInfo();
							Save(false);
						}
						break;
					//Delete node
			...
			void AddChildrenWindowRect(TimeManagement.TimeLayer layer)
			{
			...
			public void DrawNodeWindow(int id)
			{
				...
				if (drawLayer != m_saveLayers[0])
				{
					...
					if (GUI.changed)
					{
						Undo.RecordObject(m_dataInstance, "Layer changed");
			...
			void DrawLine(TimeManagement.TimeLayer layer)
			{
				//子供がいない場合終了
