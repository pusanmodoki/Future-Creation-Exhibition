...
namespace AI
{
	...
	public class AIPatrolPoints
	{
		...
		static readonly Color[] m_dReadOnlyColorTable = new Color[13]
		{
		...
		public void OnDrawGizmos()
		{
			...
			if ((!UnityEditor.EditorApplication.isPlaying
				...
				|| m_positions == null)
				&& UnityEditor.EditorApplication.timeSinceStartup > m_dDrawTime)
			{
				Start();
				m_dDrawTime = UnityEditor.EditorApplication.timeSinceStartup + 1.0f;
			}
			...
			for (int i = 0, max = m_positions.Length; i < max; ++i)
			{
				...
				foreach (var next in m_points[i].nextInfos)
				{
					...
					Gizmos.DrawCube(m_positions[next.nextElementIndex] + down + Vector3.up * m_dPointsDrawHalfDistance, Vector3.one * 0.1f);
					...
					Gizmos.DrawCube(m_positions[next.nextElementIndex] + down, Vector3.one * 0.1f);

					Gizmos.DrawLine(m_positions[i] + Vector3.up * 3.0f + down, m_positions[next.nextElementIndex] + Vector3.up * 3.0f + down);
					Gizmos.DrawCube(m_positions[next.nextElementIndex] + Vector3.up * 3.0f + down, Vector3.one * 0.1f);
